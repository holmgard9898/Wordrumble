...
const bgPreviews: Record<GameBackground, React.ReactNode> = {
  default: (
    <div className="w-full h-full rounded-xl" style={{ background: 'linear-gradient(135deg, hsl(220, 60%, 12%) 0%, hsl(260, 50%, 18%) 50%, hsl(200, 55%, 15%) 100%)' }} />
  ),
  ...
  return (
    <TilePreviewRow>
      {tileColors.map((c) => {
        ...
        return (
          <div key={c} className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white"
            style={{ background: `radial-gradient(circle at 35% 30%, ${s.highlight}, ${s.bg})` }}>A</div>
        );
      })}
    </TilePreviewRow>
  );
  ...
  return (
    <TilePreviewRow>
      {tileColors.map((c) => {
        const s = BUBBLE_COLOR_STYLES[c];
        return (
          <div key={c} className="w-6 h-6 flex items-center justify-center text-[10px] font-bold text-white"
            style={{ background: s.bg, border: '1.5px solid rgba(0,0,0,0.7)', borderRadius: '2px' }}>A</div>
        );
      })}
    </TilePreviewRow>
  );
  ...
  return (
    <TilePreviewRow>
      {tileColors.map((c) => {
        const s = BUBBLE_COLOR_STYLES[c];
        return (
          <div key={c} className="w-6 h-6 rounded-full flex items-center justify-center relative"
            style={{
            ...
            <span className="text-[10px] font-black" style={{ color: s.bg, textShadow: `0 1px 0 ${s.highlight}` }}>A</span>
          </div>
        );
      })}
    </TilePreviewRow>
    ...
    <TilePreviewRow>
      {balls.map((emoji, i) => (
        <div key={i} className="w-6 h-6 rounded-full flex items-center justify-center relative">
          ...
          <span className="absolute text-[8px] font-black text-white z-10" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.9)' }}>A</span>
        </div>
      ))}
    </TilePreviewRow>
  );
...
const tilePreviews: Record<TileStyle, React.ReactNode> = {
  bubble: <BubblePreview />,
  rubik: <RubikPreview />,
  shapes: <ShapesPreview />,
  soapbubble: <SoapBubblePreview />,
...
const Shop = () => {
  ...
  const [bgStart, setBgStart] = useState(() => {
    const idx = bgShopItems.findIndex((o) => o.bgId === settings.background);
    return Math.max(0, Math.min(idx, bgShopItems.length - VISIBLE));
  });
  const canLeft = bgStart > 0;
  const canRight = bgStart + VISIBLE < bgShopItems.length;
  const visibleBgs = bgShopItems.slice(bgStart, bgStart + VISIBLE);

  // Tile carousel
  const TILE_VISIBLE = 3;
  const [tileStart, setTileStart] = useState(() => {
    const idx = tileShopItems.findIndex((o) => o.tileId === settings.tileStyle);
    return Math.max(0, Math.min(idx, tileShopItems.length - TILE_VISIBLE));
  });
  const canTileLeft = tileStart > 0;
  const canTileRight = tileStart + TILE_VISIBLE < tileShopItems.length;
  ...
  const handleBgClick = (item: typeof bgShopItems[0]) => {
    ...
    if (isUnlocked(item.id)) {
      updateSettings({ background: item.bgId });
    } else {
      setInfoItem({ name: item.name, description: item.unlockDescription, method: item.unlockMethod, cost: item.cost, itemId: item.id });
    }
  };
  ...
  const handleTileClick = (item: typeof tileShopItems[0]) => {
    playClick();
    if (isUnlocked(item.id)) {
      updateSettings({ tileStyle: item.tileId });
    } else {
      setInfoItem({ name: item.name, description: item.unlockDescription, method: item.unlockMethod, cost: item.cost, itemId: item.id });
    }
  };
  ...
  const handleUnlock = () => {
    if (!infoItem) return;
    if (infoItem.method === 'coins' && infoItem.cost) {
      if (spendCoins(infoItem.cost)) {
        unlock(infoItem.itemId);
        setInfoItem(null);
      }
    } else if (infoItem.method === 'ad') {
      // Simulate watching an ad
      unlock(infoItem.itemId);
      setInfoItem(null);
    }
    // achievement items can't be manually unlocked
  };

  const handleWatchAd = () => {
    playClick();
    // Simulate watching an ad for 0.5 coins
    addCoins(0.5);
  };
    ...
    >
      <div className="aspect-[16/10] relative">
        ...
        {locked && (
          <div className="absolute inset-0 bg-black/50 rounded-xl flex items-center justify-center">
            {/* intentionally empty — icons in corner */}
          </div>
        )}
      </div>
      ...
      {locked && (
        <div className="absolute top-2 right-2 flex items-center gap-1">
          <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }}>
            <Lock className="w-3 h-3 text-white/60" />
          </div>
          <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }}>
            <UnlockMethodIcon method={unlockMethod} />
          </div>
        </div>
      )}
    </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-3 gap-3 flex-1">
              {visibleBgs.map((item) => {
                ...
                const active = !locked && settings.background === item.bgId;
                return renderCard(
                  { id: item.id, name: item.name, preview: bgPreviews[item.bgId] },
                  active, locked, item.unlockMethod,
                  () => handleBgClick(item),
                );
              })}
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-3 gap-3 flex-1">
              {visibleTiles.map((item) => {
                const locked = !isUnlocked(item.id);
                const active = !locked && settings.tileStyle === item.tileId;
                return renderCard(
                  { id: item.id, name: item.name, preview: tilePreviews[item.tileId] },
                  active, locked, item.unlockMethod,
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {miscShopItems.map((item) => (
              <button
                key={item.id}
                onClick={() => item.type === 'action' ? handleWatchAd() : playClick()}
                disabled={item.type === 'coming-soon'}
                className="relative rounded-2xl overflow-hidden transition-all hover:scale-[1.03] active:scale-[0.97] disabled:opacity-50 disabled:hover:scale-100"
                style={{
              ...
              >
                <div className="aspect-[4/3] flex flex-col items-center justify-center gap-1 p-3">
                  ...
                  <span className="text-white/50 text-[11px] leading-tight text-center">{item.description}</span>
                </div>
                {item.type === 'coming-soon' && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center rounded-2xl">
                    <span className="text-white/80 text-xs font-bold uppercase tracking-wider">Kommer snart</span>
                  </div>
                )}
              </button>
            ))}
      ...
      <Dialog open={!!infoItem} onOpenChange={(open) => !open && setInfoItem(null)}>
        <DialogContent className="max-w-xs rounded-2xl border-white/10" style={{ background: 'rgba(15,23,42,0.95)', backdropFilter: 'blur(20px)' }}>
          <DialogHeader>
            ...
            <DialogDescription className="text-white/70 text-center text-sm pt-2">
          ...
          <div className="flex flex-col gap-2 mt-2">
            {infoItem?.method === 'coins' && (
              <Button
                ...
                disabled={coins < (infoItem.cost ?? 0)}
                className="w-full gap-2"
                style={{ background: coins >= (infoItem.cost ?? 0) ? 'rgba(139,92,246,0.8)' : 'rgba(100,100,100,0.5)' }}
              >
                <Coins className="w-4 h-4 text-yellow-400" />
                Köp för {infoItem.cost} coins
              </Button>
            )}
            {infoItem?.method === 'ad' && (
              <Button onClick={handleUnlock} className="w-full gap-2" style={{ background: 'rgba(59,130,246,0.8)' }}>
                <Video className="w-4 h-4" />
                Se reklamvideo & lås upp
              </Button>
            )}
            {infoItem?.method === 'achievement' && (
              <div className="text-center text-white/50 text-sm py-2">
                <HelpCircle className="w-5 h-5 mx-auto mb-1 text-purple-400" />
                Denna låses upp genom prestation i spelet.
              </div>
            )}
            <Button onClick={() => setInfoItem(null)} variant="ghost" className="w-full text-white/60 hover:text-white hover:bg-white/10">
              Stäng
            </Button>
          </div>
        </DialogContent>
