...
const MainMenu = () => {
  ...
  const goAdventure = () => {
    playClick();
    let last = 1;
    try { last = parseInt(localStorage.getItem('wr-adventure-last-map') || '1', 10) || 1; } catch { /* ignore */ }
    if (last === 2) navigate('/adventure/map/2');
    else if (last === 3) navigate('/adventure/map/3');
    else navigate('/adventure');
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-4 w-full max-w-xs">
        ...
        <MenuButton onClick={goAdventure} icon={<Map className="w-5 h-5" />} label={t.adventureTitle} gradient="amber" />
