  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 ${bg.className}`} style={bg.style}>
      ...
      <div className="w-full max-w-xs flex flex-col gap-4">
        ...
        <Button onClick={handleFacebookAuth} size="lg" className="h-14 text-lg bg-[#1877F2] text-white hover:bg-[#166FE5] gap-3">
        ...
        <form onSubmit={handleEmailAuth} className="flex flex-col gap-3">
          {!isLogin && (
            <div className="relative">
              ...
              <Input type="text" placeholder={t.displayName} value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="pl-10 bg-white/10 border-white/20 text-white placeholder:text-white/40" />
            </div>
          )}
          <div className="relative">
            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
            <Input type="email" placeholder={t.email} value={email} onChange={(e) => setEmail(e.target.value)} required className="pl-10 bg-white/10 border-white/20 text-white placeholder:text-white/40" />
          </div>
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
            <Input type="password" placeholder={t.password} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="pl-10 bg-white/10 border-white/20 text-white placeholder:text-white/40" />
          </div>
          <Button type="submit" disabled={loading} size="lg" className="h-14 text-lg bg-gradient-to-r from-purple-600 to-pink-500 hover:from-purple-500 hover:to-pink-400 text-white">
            {loading ? t.wait : isLogin ? t.logIn : t.createAccount}
          </Button>
        </form>

        <button onClick={() => { playClick(); setIsLogin(!isLogin); }} className="text-white/50 text-sm hover:text-white/80 transition-colors">
