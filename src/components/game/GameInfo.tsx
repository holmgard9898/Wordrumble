...
export function GameInfo({
  ...
  const MODE_BADGE: Record<GameMode, { icon: React.ReactNode; label: string; color: string; border: string } | null> = {
    classic: null,
    surge: { icon: <Zap className="w-4 h-4 text-yellow-400" />, label: t.modeSurge, color: 'rgba(234,179,8,0.2)', border: 'rgba(234,179,8,0.3)' },
    fiveplus: { icon: <Hash className="w-4 h-4 text-cyan-400" />, label: t.modeFiveplus, color: 'rgba(34,211,238,0.2)', border: 'rgba(34,211,238,0.3)' },
    bomb: { icon: <Bomb className="w-4 h-4 text-red-400" />, label: t.modeBomb, color: 'rgba(239,68,68,0.2)', border: 'rgba(239,68,68,0.3)' },
    oneword: { icon: <Target className="w-4 h-4 text-emerald-400" />, label: t.modeOneword, color: 'rgba(16,185,129,0.2)', border: 'rgba(16,185,129,0.3)' },
  };
    ...
    <div className="flex flex-col gap-3 w-full max-w-xs">
      ...
      {lastFoundWord && (
        <div className="rounded-xl p-2 md:p-3 text-center animate-pulse" style={{ background: 'rgba(34,197,94,0.2)', border: '1px solid rgba(34,197,94,0.4)' }}>
          <div className="text-[10px] md:text-xs uppercase tracking-wider opacity-70 text-green-300 mb-1">{t.wordFound}</div>
